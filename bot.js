...
const executeCommand = async (chatId, command, host, time, username) => {
    ...
    const startMessage = {
        status: "🚀Successfully🚀", pid, website: host, time: `${time} Giây`, caller: username, startTime, maxSlots: maxSlot,
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('http://') || text.startsWith('https://')) {
        ...
        const userAttack = currentAttacks.get(chatId);
        if (userAttack) {
            const remainingTime = maxTimeAttacks - (Date.now() - userAttack.startTime) / 1000;
