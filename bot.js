...
const executeCommand = async (chatId, command, host, time, username) => {
    ...
    const startMessage = {
        status: "🚀 Successfully 🚀", pid, website: host, time: `${time} Seconds`,
        caller: `@${username}`, maxSlots: maxSlot, startTime,
    ...
    child.on('close', () => {
        ...
        const completeMessage = {
            status: "✅ Process Completed", pid, website: host, time: `${time} Seconds`,
            caller: `@${username}`, endTime
        };
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('exe ') && isAdmin) {
        ...
        child.on('close', () => sendJsonMessage(chatId, { status: "🚀 Command Result", command, output }));
        return;
    }
